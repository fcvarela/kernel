...
uint8_t read_serial_register(uint16_t offset) { return port_inb(SERIAL_REGISTER_BASE + offset * SERIAL_REGISTER_STRIDE); }

void write_serial_register(uint16_t offset, uint8_t d) { port_outb(SERIAL_REGISTER_BASE + offset * SERIAL_REGISTER_STRIDE, d); }
...
EFI_STATUS kserial_init(EFI_SYSTEM_TABLE *st) {
  ...
  while ((read_serial_register(R_UART_LSR) & (B_UART_LSR_TEMT | B_UART_LSR_TXRDY)) != (B_UART_LSR_TEMT | B_UART_LSR_TXRDY)) {
  }
  ...
  write_serial_register(R_UART_FCR, (uint8_t)(SERIAL_LINE_CONTROL & (B_UART_FCR_FIFOE | B_UART_FCR_FIFO64)));
...
bool serial_port_writable() {
  if (SERIAL_USE_HW_FLOW_CONTROL) {
    if (SERIAL_DETECT_CABLE) {
      ...
      return (bool)((read_serial_register(R_UART_MSR) & (B_UART_MSR_DSR | B_UART_MSR_CTS)) == (B_UART_MSR_DSR | B_UART_MSR_CTS));
    } else {
      ...
      return (bool)((read_serial_register(R_UART_MSR) & (B_UART_MSR_DSR | B_UART_MSR_CTS)) != (B_UART_MSR_DSR));
    }
  }
  return true;
}
